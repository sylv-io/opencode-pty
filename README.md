...
> - **Multiline scripts**: When writing multiline bash scripts via `pty_write`, tree-sitter is used to extract only actual executable commands (matching the built-in bash tool behavior). Variable assignments, function definitions, and loop structures do not generate permission prompts.
